...
// para displays de 4 lineas el contador acendente debe de ser de 1-99.
{
	...
	for (byte count=1;count<100;count++)
	{
		...
		{
			UpdateCursor(i);				// se mueve a la siguiente línea, si es necesario
			LCD_Char(ch);					// muestra el actual caracter ascii
			ch = GetNextChar(ch);				// actualiza añ siguiente caracter
			msDelay(60);					// configura la velocidad de animación
		}
	}
}
